Shader "SFS/VoxelTree"
{
	Properties
	{
		...
		_BendAmount1("Bend Amount", Range( 0 , 0.02)) = 0.01
	...
	SubShader
	{
		...
		Pass
		{
			...
			VertexOutput VertexFunction( VertexInput v  )
			{
				...
				float temp_output_92_0 = ( ( cos( ( ( ( ase_worldPos.x + ase_worldPos.z ) * 0.06 ) + _TimeParameters.x ) ) * v.vertex.xyz.y ) * _BendAmount1 );
				float4 appendResult93 = (float4(temp_output_92_0 , 0.0 , temp_output_92_0 , 0.0));
				float4 break96 = mul( appendResult93, GetObjectToWorldMatrix() );
				...
				#endif
				float3 vertexValue = rotatedValue100;
				#ifdef ASE_ABSOLUTE_VERTEX_POS
